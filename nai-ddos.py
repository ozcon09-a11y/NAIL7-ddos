...
def build_session(timeout, keepalive=True, verify_tls=True):
    ...
    s.mount("https://", adapter)                                                            s.headers.update({
...
def worker(idx, args, job_q: queue.Queue, metrics: Metrics, start_ts, end_ts):
    session = build_session(timeout=args.timeout, keepalive=not args.no_keepalive, veri>
    rng = random.Random(idx ^ int(time.time()))                                             # simple log pulse each second
    last_log = time.time()
...
def main():
    ...
    payload = None
    if args.payload:                                                                            # rough parse: try JSON then fall back to raw string
        import json
        try:
    ...
    for i in range(args.threads):
        t = threading.Thread(target=worker, args=(i, args, job_q, metrics, start_ts, en>
        t.start()
        threads.append(t)
